...
class AdditiveAttention(nn.Module):
    def __init__(self, hidden_dim: int):
        ...
        # Linear layers for additive attention (Bahdanau attention)
    ...
    def forward(
        ...
        decoder_hidden_repeated = decoder_hidden.unsqueeze(1).repeat(1, src_seq_len, 1)
        
        # Concatenate encoder outputs and decoder hidden
        combined = torch.cat((encoder_outputs, decoder_hidden_repeated), dim=2)
        
        # Compute attention scores
        attn_hidden = torch.tanh(self.attn_hidden(combined))
        attention_scores = self.attn_score(attn_hidden).squeeze(2)
...
class DecoderWithAdditiveAttention(nn.Module):
    def __init__(
        self, 
        ...
        super(DecoderWithAdditiveAttention, self).__init__()
...
class EncoderDecoderAdditiveAttention(nn.Module):
    def __init__(
        self, 
        encoder: EncoderWithAttention, 
        decoder: DecoderWithAdditiveAttention, 
        device: torch.device
    ):
        super(EncoderDecoderAdditiveAttention, self).__init__()
